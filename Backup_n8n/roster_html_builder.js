...
.hdr{
  ...
  padding:14px 12px 12px;text-align:center;position:sticky;top:0;z-index:10;
  flex-shrink:0;
}
.thm-btn{
...
#dp{
  ...
  /* let the native picker trigger on direct tap */
  -webkit-appearance:none;
}

/* TABLE WRAPPER */
.body{padding:12px 8px 8px;overflow-y:auto;flex:1;}
.tbl-w{
...
<div class="wrap">
  <div class="hdr">
    ...
    <div class="d-row">
      ...
      <span class="d-lbl">${displayDate}<input type="date" id="dp" value="${selectedDate}" onchange="goDate(this.value)"></span>
...
let theme='dark';
(function(){
  // 1. prefer theme from URL param (survives navigation)
  const urlP=new URLSearchParams(window.location.search);
  const urlTheme=urlP.get('theme');
  if(urlTheme==='light'||urlTheme==='dark'){theme=urlTheme;}
  else{try{theme=localStorage.getItem('roster-theme')||'dark';}catch(e){}}
  applyT(theme);
})();
function applyT(t){
...
function toggleTheme(){
  ...
  // keep URL in sync so nav-btn arrows preserve new theme
  const u=new URL(window.location.href);
  u.searchParams.set('theme',theme);
  history.replaceState(null,'',u.toString());
}
// openDP kept as no-op for backwards compat; picker is now directly overlaid
function openDP(){}
let gVis=false;
function toggleG(){
  gVis=!gVis;
...
function goDate(d){
  ...
  window.location.href=window.location.pathname+'?date='+encodeURIComponent(d)+'&theme='+encodeURIComponent(theme)+(tok?'&token='+encodeURIComponent(tok):'');
}
...
// ensure date input reflects any URL theme
document.getElementById('dp').addEventListener('change',function(){goDate(this.value);});
</script>
</body>
</html>`;
